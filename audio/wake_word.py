...
class PorcupineWakeWord:
    ...
    def _resolve_openwakeword_paths(self) -> list[str]:
        ...
        try:
            ...
            log.warning("openwakeword_import_failed", error=str(e))
            return []
    ...
    def load(self):
        ...
        if self._engine == "openwakeword":
            try:
                ...
                log.warning(
                    "openwakeword_load_failed",
                    error=str(e),
                    note="falling back to porcupine if key available",
                )
                self._engine = "porcupine"
        ...
        if self._engine == "porcupine":
            ...
            if not access_key:
                log.warning(
                    "porcupine_key_missing", note="PICOVOICE_ACCESS_KEY is empty"
                )
            else:
                import pvporcupine
        ...
        if self._openwakeword_model is None and self._porcupine is None:
            raise RuntimeError(
                "Wake word engine tidak aktif: model openWakeWord tidak ditemukan dan "
                "PICOVOICE_ACCESS_KEY kosong. Pasang paket openwakeword + set OPENWAKEWORD_MODEL "
                "yang valid, atau set WAKE_WORD_ENGINE=porcupine dan PICOVOICE_ACCESS_KEY di .env."
            )
